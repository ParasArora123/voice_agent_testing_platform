...
def start_deepgram_stream(audio_queue, transcript_queue):
    """
    ...
    Once combined, the full question/combined transcript is put in the transcript queue (to 
    eventually be used by the LLM to generate an answer in the inference layer).
    
    Note that speech_final and utterance_end are used to determine when the user is naturally done
    speaking and ready for the voice agent to answer. This is just a heuristic and not perfect.
    """
